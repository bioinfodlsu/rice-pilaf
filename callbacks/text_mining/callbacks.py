...
def init_callback(app):
    ...
    @app.callback(
        ...
        Input({"type": "example-text-mining", "description": ALL}, "n_clicks"),
    ...
    def set_input_fields_with_preset_input(example_text_mining_n_clicks):
        ...
        if ctx.triggered_id and not all(
            val == 0 for val in example_text_mining_n_clicks
        ):
    ...
    @app.callback(
        ...
        Output("text-mining-submitted-query", "data", allow_duplicate=True),
        Output("text-mining-submitted-filter", "data", allow_duplicate=True),
        Input("text-mining-submit", "n_clicks"),
    ...
    def submit_text_mining_input(
        text_mining_submitted_n_clicks,
        text_mining_query_n_submit,
    ...
    def display_text_mining_results(
        text_mining_is_submitted,
        homepage_is_submitted,
        genomic_intervals,
        text_mining_submitted_query,
        text_mining_submitted_filter,
    ):
        """
    ...
    def download_text_mining_table_to_csv(
        download_n_clicks, text_mining_df, submitted_query
    ):
        """
        ...
        if download_n_clicks >= 1:
            ...
            return dcc.send_data_frame(
                df.to_csv,
                f"[{submitted_query}] Text Mining Analysis Table.csv",
                index=False,
            )
