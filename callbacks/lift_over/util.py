...
from ..links_util import (
    ...
    get_rgi_genecard_link_single_str,
    get_uniprot_link,
)
...
def to_genomic_interval_str(genomic_interval_str):
    ...
    genomic_interval_str = (
        str(genomic_interval_tuple.chrom)
        + ":"
        + str(genomic_interval_tuple.start)
        + "-"
        + str(genomic_interval_tuple.stop)
    )
    ...
    try:
        ...
        for character_major in sorted(mapping[gene]):
            ...
            for character_minor in sorted(mapping[gene][character_major]):
                pubs = [
                    "<li>" + get_doi_link_single_str(pub) + "</li>"
                    for pub in sorted(mapping[gene][character_major][character_minor])
                ]
                qtaro_str += (
                    "<li>" + character_minor + "<ul>" + "".join(pubs) + "</ul></li>"
                )
            qtaro_str += "</ul></li><br>"
...
def get_pubmed_entry(gene, pubmed_mapping):
    try:
        pubmed_ids = [
            get_pubmed_link_single_str(pubmed_id[0])
...
def get_interpro_entry(gene, interpro_mapping, iric_mapping):
    try:
        return "<br><br>".join(
            [
                get_interpro_link_single_str(entry[1], entry[0])
...
def get_pfam_entry(gene, pfam_mapping, iric_mapping):
    try:
        return "<br><br>".join(
            [
                get_pfam_link_single_str(entry[1], entry[0])
...
def get_nb_ortholog(gene, nb_ortholog_mapping):
    if nb_ortholog_mapping[gene]:
        return "<br>".join(
            map(get_rgi_genecard_link_single_str, nb_ortholog_mapping[gene])
        )
...
def get_genes_in_Nb(genomic_intervals):
    ...
    if genomic_intervals:
        temp_output_dir = get_path_to_temp(genomic_intervals, Constants.TEMP_LIFT_OVER)
    ...
    NB_GENES_FILENAME_WITH_TIMESTAMP = append_timestamp_to_filename(NB_GENES_FILENAME)
...
def get_genes_in_Nb_if_not_exist(genomic_intervals):
    ...
    with open(f"{Constants.OGI_MAPPING}/Nb_to_ogi.pickle", "rb") as ogi_file, open(
        Constants.QTARO_DICTIONARY, "rb"
    ) as qtaro_file, open(
        ...
        for nb_interval in nb_intervals:
            ...
            gene_ids_in_interval = [
                sanitize_gene_id(gene.id) for gene in genes_in_interval
            ]
            ...
            df = pd.DataFrame(
                {
                    ...
                    "QTL Analyses": get_qtaro_entries(
                        gene_ids_in_interval, qtaro_mapping
                    ),
                    "PubMed Article IDs": get_pubmed_entries(
                        gene_ids_in_interval, pubmed_mapping
                    ),
                    "InterPro": get_interpro_entries(
                        gene_ids_in_interval, interpro_mapping, iric_mapping
                    ),
                    "Pfam": get_pfam_entries(
                        gene_ids_in_interval, pfam_mapping, iric_mapping
                    ),
    ...
    try:
        ...
        table["UniProtKB/Swiss-Prot"] = get_uniprot_link(table, "UniProtKB/Swiss-Prot")

        table = table.fillna(NULL_PLACEHOLDER).drop_duplicates().sort_values("Name")
...
def get_genes_in_other_ref(ref, nb_intervals):
    ...
    db_align = gffutils.FeatureDB(f"{Constants.ALIGNMENTS}/Nb_{ref}/Nb_{ref}.gff.db")

    # Get corresponding intervals on ref
    db_annotation = gffutils.FeatureDB(f"{Constants.ANNOTATIONS}/{ref}/{ref}.gff.db")
...
def get_all_genes(refs, genomic_intervals):
    ...
    make_dir(temp_output_dir)
    ALL_GENES_FILENAME_WITH_TIMESTAMP = append_timestamp_to_filename(ALL_GENES_FILENAME)
...
def get_all_genes_if_not_exist(refs, genomic_intervals):
    ...
    common_genes = (
        common_genes.rename(columns={"Name": "Nipponbare"})
...
def get_unique_genes_in_other_ref(refs, ref, genomic_intervals):
    ...
    unique_genes = all_genes.loc[
        (all_genes["Nipponbare"] == NULL_PLACEHOLDER)
        & (all_genes[ref] != NULL_PLACEHOLDER)
    ]
    ...
    unique_genes = unique_genes[FRONT_FACING_COLUMNS + ["Ortholog in Nipponbare"]]
    ...
    unique_genes = (
        unique_genes.fillna(NULL_PLACEHOLDER).drop_duplicates().sort_values("Name")
    )

    if unique_genes.shape[0] == 0:
        return create_empty_df_with_cols(
            FRONT_FACING_COLUMNS + ["Ortholog in Nipponbare"]
        )
