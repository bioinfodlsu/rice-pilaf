...
LINK_ICON_DASH = html.Span(
    [
        " ",
        html.I(
            id="demo-link", className="fa-solid fa-up-right-from-square fa-2xs ms-1"
        ),
    ],
...
def construct_link(link, text, dash=False):
    if dash:
        return dcc.Link([text, LINK_ICON_DASH], href=link, target="_blank")
...
def get_to_po_link(result, id_col):
    LINK = "https://www.ebi.ac.uk/ols4/ontologies/to/classes/http%253A%252F%252Fpurl.obolibrary.org%252Fobo%252F" + result[
        id_col
    ].str.replace(
        ":", "_"
    )
...
def get_pubmed_link_single_str(pubmed, dash=False):
    LINK = "https://pubmed.ncbi.nlm.nih.gov/" + pubmed + "/entry"
