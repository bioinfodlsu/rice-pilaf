...
def convert_msu_to_transcript(cluster_dir, mapping_file, output_dir):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    for cluster_file in os.listdir(cluster_dir):
        with open(f'{cluster_dir}/{cluster_file}') as clusters, open(mapping_file, 'rb') as mapping, open(f'{output_dir}/{cluster_file[:-len(".txt")]}_transcript_id.txt', 'w') as output_file:
