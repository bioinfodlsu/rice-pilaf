...
def detect_modules(edge_list_file, module_list_dir, density_threshold, affinity_threshold, closeness_threshold):
    G = None
    ...
    coms = algorithms.coach(
        G, density_threshold=density_threshold, affinity_threshold=affinity_threshold, closeness_threshold=closeness_threshold)

    if not os.path.exists(module_list_dir):
        os.makedirs(module_list_dir)

    readwrite.write_community_csv(
        coms, f'{module_list_dir}/coach-int-module-list.csv', '\t')
...
if __name__ == '__main__':
    ...
    detect_modules(args.edge_list_file, args.module_list_dir,
                   args.density_threshold, args.affinity_threshold, args.closeness_threshold)
