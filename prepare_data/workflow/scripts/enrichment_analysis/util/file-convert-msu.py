...
def convert_msu(msu_id_file, mapping_file, output_dir, target_id, skip_no_matches):
    ...
    with open(msu_id_file) as msu_file, open(mapping_file, "rb") as mapping, open(
        f"{output_dir}/{target_id}/{output_file_name}", "w"
    ) as output:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_dir",
        help="output directory for the file containing the equivalent IDs after conversion",
    )
    parser.add_argument("target_id", help='either "entrez" or "transcript"')
    parser.add_argument(
        "--skip_no_matches",
        action="store_true",
