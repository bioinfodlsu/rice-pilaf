...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'pathway_geneset_file', help='pickled dictionary mapping pathways to the genes involved in the pathways'
    )
    parser.add_argument(
        'output_dir', help='output directory for the pickled dictionary mapping genes to their pathways'
    )
    parser.add_argument(
        'transcript_to_msu_files', help='pickled dictionaties mapping transcript IDs to MSU accessions', nargs='+')

    args = parser.parse_args()

    export_mapping(map_genes_to_pathway(args.pathway_geneset_file, args.transcript_to_msu_files),
                   args.output_dir)
