...
def map_using_rb_dp(
    msu_to_transcript_dict, msu_to_rap_dict, rap_to_transcript_dict, gene
):
    transcript_ids = set()
    ...
    msu_to_transcript_dict[gene] = msu_to_transcript_dict[gene].union(transcript_ids)


def map_no_transcript_id(
    msu_to_transcript_dict,
    msu_to_rap_dict,
    rap_to_transcript_dict,
    no_transcript_id_file,
):
...
def save_msu_transcript_mapping(msu_to_transcript_dict, output_dir):
    ...
    with open(f"{output_dir}/msu-to-transcript-id.pickle", "wb") as handle:
        pickle.dump(msu_to_transcript_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "riceidconverter_msu_to_transcript_id_file",
        help="text file containing MSU accessions and their respective KEGG transcript IDs as mapped via riceidcoverter",
    )
    parser.add_argument(
        "riceidconverter_no_transcript_id_file",
        help="text file containing the list of MSU accessions that cannot be mapped by riceidcoverter to KEGG transcript IDs",
    )
    parser.add_argument(
        "rap_to_msu_file", help="text file mapping RAP to MSU accessions"
    )
    parser.add_argument(
        "rap_to_transcript_file",
        help="text file mapping RAP accessions to KEGG transcript IDs",
    )
    parser.add_argument(
        "output_dir",
    ...
    load_msu_to_transcript(
        msu_to_transcript_dict, args.riceidconverter_msu_to_transcript_id_file
    )
    load_msu_to_rap(msu_to_rap_dict, args.rap_to_msu_file)
    load_rap_to_transcript(rap_to_transcript_dict, args.rap_to_transcript_file)

    map_no_transcript_id(
        msu_to_transcript_dict,
        msu_to_rap_dict,
        rap_to_transcript_dict,
