...
def from_rap_db(rap_db_file, all_genes_file, msu_to_transcript_file):
    ...
    transcript_to_msu_mapping = convert_transcript_to_msu(msu_to_transcript_file)
...
def save_to_csv(output_dir, merged_df):
    ...
    merged_df.to_csv(
        f"{output_dir}/go-annotations.tsv", sep="\t", index=False, header=False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "all_genes_file",
        help="text file containing the RAP-DB accessions of all the genes of interest",
    )
    parser.add_argument(
        "msu_to_transcript_file",
        help="pickled dictionary mapping MSU accessions to KEGG transcript IDs",
    )
    parser.add_argument(
        "output_dir",
        help="output directory for the TSV file mapping MSU accessions to GO term IDs",
    )
