...
def save_transcript_msu_mapping(transcript_to_msu, output_dir):
    ...
    with open(f"{output_dir}/transcript-to-msu-id.pickle", "wb") as handle:
        pickle.dump(transcript_to_msu, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="pickled dictionary mapping MSU accessions to their respective KEGG transcript IDs",
    )
    parser.add_argument(
        "output_dir",
        help="output directory for the pickled dictionary mapping KEGG transcript IDs to their respective MSU accessions",
    )
