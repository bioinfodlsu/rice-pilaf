...
def generate_mapping(mapping, edge_list_file, int_edge_list_node_mapping_file):
    G = None
    with open(edge_list_file, 'r') as edge_list, open(int_edge_list_node_mapping_file, 'rb') as int_edge_list_node_map:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'edge_list_file', help='text file corresponding to the edge list with the node labels converted to integers')
    parser.add_argument(
        'int_edge_list_node_mapping_file', help='pickled dictionary mapping the integer node labels to the (original) string labels'
    )
    parser.add_argument(
        'output_dir', help='output directory for the pickled dictionary mapping the node labels in the networkx integer-indexed graph to their (original) string labels'
    )

    args = parser.parse_args()

    mapping = {}
    generate_mapping(mapping, args.edge_list_file,
                     args.int_edge_list_node_mapping_file)
