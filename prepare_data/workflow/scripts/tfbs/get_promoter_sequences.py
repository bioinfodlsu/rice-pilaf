...
def main(genome,gff_db,upstream_win_len,downstream_win_len,out_fasta,promoter_gene_map_f):
    ...
    for gene in genes:
        if gene.strand == '+':
            ...
            promoter_seq_id = gene.seqid+":"+str(gene.start-1-upstream_win_len)+"-"+str(gene.start-1+downstream_win_len)
        ...
        promoters.append(promoter_seq_rec)
        promoter_gene_map.write(promoter_seq_id+"\t"+gene.attributes["ID"][0]+"\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('promoter_gene_map',help='path to file containing promoter-to-gene map')

    args= parser.parse_args()
    main(args.genome, args.gff_db,args.upstream_win_len, args.downstream_win_len,
         args.out_fasta, args.promoter_gene_map)