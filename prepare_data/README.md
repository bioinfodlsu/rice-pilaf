...
- [Module Display](https://github.com/bioinfodlsu/rice-pilaf/tree/main/prepare_data#module-display)
...
This recipe assumes that the module of interest is the 100<sup>th</sup> module (as specified using the `-i` parameter) and uses the `dosaSPIA.RData` file generated by SPIA from the KEGG pathway data files for the organism `dosa` (downloaded on May 11, 2023):
...
### 1. Converting Module List to Edge List

This recipe converts a module list generated by a module detection algorithm into an edge list in view of the coexpression network. In the module list generated by a module detection algorithm, only the nodes comprising the module are listed. However, in order to display the module as a graph, the edges connecting these modules have to be recovered from the coexpression network.

```
python network_display/convert-modules-to-edge-lists.py ../../../static/raw_data/networks/OS-CX.txt ../../../static/raw_data/networks_modules/OS-CX/module_list/{ALGO}/{PARAM}/{ALGO}-module-list.tsv ../../../static/app_data/networks_display/OS-CX/{ALGO}/modules/{PARAM}
```

Replace `ALGO` with the algorithm (`fox`, `demon`, `coach`, or `clusterone`) and `PARAM` with the name of the directory containing the module list after running the algorithm with the specified parameter:
- For example, if `ALGO` is `clusterone` and the parameter (minimum density) is 0.3, then `PARAM` is `30`.

### 2. Mapping Modules to Genes

This recipe maps the modules to the genes belonging to them. This is used in constructing the link to the KEGG pathway visualization, which requires the genes belonging to the module of interest to be singled out from the other genes in the pathway.

```
python network_display/map-modules-to-genes.py ../../../static/raw_data/networks_modules/OS-CX/module_list/{ALGO}/{PARAM}/{ALGO}-module-list.tsv ../../../static/app_data/networks_display/OS-CX/{ALGO}/modules_to_genes/{PARAM}
```
